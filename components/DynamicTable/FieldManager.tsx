  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogTrigger asChild>
        <button className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-gray-600 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">
          <Settings2 size={12} />
          Customize Fields
        </button>
      </DialogTrigger>
      {editingFieldIndex !== null ? (
        <DialogContent className="sm:max-w-[500px] max-h-[85vh] flex flex-col p-0 gap-0 bg-white rounded-xl overflow-hidden">
          {(() => {
                  ...
                  <div className="space-y-3">
                    {field.options?.map((opt: SelectOption) => (
                      <div key={opt.id} className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() =>
                            removeOption(editingFieldIndex, opt.id)
                          }
